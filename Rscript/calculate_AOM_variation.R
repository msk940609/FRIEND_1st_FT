...
fm_bromo_sel$Bromo2=fm_bromo_sel$Bromo.Inty*100000
...
ggplot(fm_bromo_sel,aes(x=as.factor(cnt), y=log10(Bromo2)))+
  ...
  ggsave(filename("Meanadundance_hist_log10"), height = 10, width = 50, units = "cm", dpi = 300)
...
mol_rich_n=mol_rich %>% inner_join(FT_envi_sel[,c("Sample","Group","No","Date","PM2.5","OC","POC","SOC","WSOC")])
...
aom_var_m
aom_var_m$No=as.numeric(aom_var_m$No)

aom_var_m_env=aom_var_m %>% inner_join(FT_envi[,c("Group","No","PM2.5","OC")])

minoc=FT_envi_sel[ , .SD[which.min(OC)], by = Group]
minoc=minoc[,c("Group","OC")] %>% `colnames<-`(c("Group","OCmin"))
minpm=FT_envi_sel[ , .SD[which.min(PM2.5)], by = Group]
minpm=minpm[,c("Group","PM2.5")] %>% `colnames<-`(c("Group","PMmin"))


min_envi=FT_envi_sel %>% filter(Sample%in%c("Ulaanbaatar_13","Beijing_25","Noto_17","Seosan_23","Seoul_1"))
min_envi=min_envi[,c("Group","OC","PM2.5")] %>% `colnames<-`(c("Group","OCmin","PMmin"))

#aom_var_m_env=aom_var_m_env %>% inner_join(min_envi)
aom_var_m_env=aom_var_m_env %>% left_join(minoc)
aom_var_m_env=aom_var_m_env %>% left_join(minpm)

aom_var_m_env$ocdif=aom_var_m_env$OC-aom_var_m_env$OCmin
...
aom_var_m_env$Group=factor(aom_var_m_env$Group,levels = c("Ulaanbaatar","Beijing","Seosan","Seoul","Noto"))

aom_var_m_env$Group2=factor(aom_var_m_env$Group,levels = c("Ulaanbaatar","Beijing","Seosan","Seoul","Noto"),
                        labels = c(expression(bold("Ulaanbaatar")),
                                   expression(bold("Beijing")),
                                   expression(bold("Seosan")),
                                   expression(bold("Seoul")),
...
ggplot(data=aom_var_m_env)+
  geom_line(data = aom_var_m_env ,aes(x= pmdif, y=value, col=variable2 ),size=0.7, na.rm = F)+
  geom_point(data = aom_var_m_env ,aes(x= pmdif, y=value,shape=variable2,col=variable2))+
  facet_wrap(.~Group, ncol = 1, scales = "free")+
  scale_y_continuous(name = "Podani's indice (Jaccard dissimilarity)")+
  #scale_y_continuous(name =expression(bold(PM[2.5]~"("*"\u03bcg/"*m^"3"*")")),expand = c(0.02,0.02),
  #                   sec.axis = sec_axis(~./1.5, name=expression(bold("OC"~"("*"\u03bcg/"*m^"3"*")")))
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.ticks.length = unit(0.15,"cm"),
        axis.ticks = element_line(size = 1.2, colour = "black"),
        axis.title.x = element_text(size = 20, colour = "black",face=2,margin = unit(c(0.1,0.6,0.1,0.1),"cm"),family = "Arial"),
        axis.text.y = element_text(size = 16, colour = "black" , face = "bold",family = "Arial"),
        axis.title.y.left = element_text(size = 20, colour = "black",face=2,margin = unit(c(0.1,0.6,0.1,0.1),"cm"),family = "Arial"),
        axis.title.y.right = element_text(size = 20, colour = "black",margin = unit(c(0.1,0.0,0.1,0.4),"cm"),family = "Arial"),
        legend.text = element_text(size = 14, colour = "black",family = "Arial",margin = unit(c(0.1,0.1,0.2,0.1),"cm"), hjust = 0.0),
  ...
  guides(col=guide_legend(order = 1,title ="", override.aes = list(shape=c(17,18))),
         shape="none")+
  ggsave(filename("AOM_variation_pm"), height = 30, width = 30, units = "cm", dpi = 300)


ggplot(data=aom_var_m_env)+
  geom_line(data = aom_var_m_env ,aes(x= ocdif, y=value, col=variable2 ),size=0.7, na.rm = F)+
  geom_point(data = aom_var_m_env ,aes(x= ocdif, y=value,shape=variable2,col=variable2))+
  facet_wrap(.~Group, ncol = 1, scales = "free")+
  scale_y_continuous(name = "Podani's indice (Jaccard dissimilarity)")+
  #scale_y_continuous(name =expression(bold(PM[2.5]~"("*"\u03bcg/"*m^"3"*")")),expand = c(0.02,0.02),
  #                   sec.axis = sec_axis(~./1.5, name=expression(bold("OC"~"("*"\u03bcg/"*m^"3"*")")))
  ...
  guides(col=guide_legend(order = 1,title ="", override.aes = list(shape=c(17,18))),
         shape="none")+
  ggsave(filename("AOM_variation_oc"), height = 30, width = 30, units = "cm", dpi = 300)
